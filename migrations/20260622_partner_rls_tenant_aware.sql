...
--   - Add tenant-scoped policies for `authenticated`: a user may only touch rows
--     of tenants they are an accepted member of. Role granularity (owner/
--     operator/viewer) stays enforced at the app layer (requireTenantPermission);
--     RLS provides defense-in-depth tenant isolation.
...
-- 2) Tenant-scoped access for authenticated users (members of the tenant).

-- tenants: members can read/update their own tenant. Creation/deletion stay
-- backend-only (service role via onboarding).
...
create policy "Members update their tenants" on tenants
...
-- partner_products / assets / leads / design_config: full access within own tenant.
drop policy if exists "Members manage their products" on partner_products;
create policy "Members manage their products" on partner_products
...
create policy "Members manage their assets" on partner_assets
  for all to authenticated using (is_tenant_member(tenant_id)) with check (is_tenant_member(tenant_id));

drop policy if exists "Members manage their leads" on partner_leads;
create policy "Members manage their leads" on partner_leads
  for all to authenticated using (is_tenant_member(tenant_id)) with check (is_tenant_member(tenant_id));

drop policy if exists "Members manage their design config" on partner_design_config;
create policy "Members manage their design config" on partner_design_config
  for all to authenticated using (is_tenant_member(tenant_id)) with check (is_tenant_member(tenant_id));

-- partner_product_variants: tenant resolved through the parent product.
drop policy if exists "Members manage their variants" on partner_product_variants;
create policy "Members manage their variants" on partner_product_variants
  for all to authenticated
  using (exists (select 1 from partner_products pp where pp.id = product_id and is_tenant_member(pp.tenant_id)))
  with check (exists (select 1 from partner_products pp where pp.id = product_id and is_tenant_member(pp.tenant_id)));
