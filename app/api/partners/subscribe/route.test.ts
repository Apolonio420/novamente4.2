...
vi.mock('@/lib/partners/subscription', () => ({
  ...
  isGrowthPromoEligible: h.promoEligible,
  resolveAnnualPriceUsd: h.resolveAnnual,
}))
...
describe('POST /api/partners/subscribe', () => {
  beforeEach(() => {
    ...
    h.promoEligible.mockResolvedValue(false)
    h.resolveAnnual.mockImplementation((_plan: string, eligible: boolean) => (eligible ? 127 : 255))
  })
  ...
  it('anual growth con promo: registra el lock en metadata SIN activar plan ni ciclo', async () => {
    h.promoEligible.mockResolvedValueOnce(true)

    const response = await POST(request({ plan: 'growth', billingCycle: 'annual' }))
    const data = await response.json()

    expect(response.status).toBe(200)
    expect(data.payment_pending).toBe(true)
    expect(data.price_usd).toBe(127)
    expect(data.promo).toEqual(expect.objectContaining({ price_usd: 127 }))

    // El lock de la promo se persiste en metadata, pero el plan/ciclo/vencimiento
    // quedan intactos hasta que el webhook confirme el pago.
    expect(h.updateTenant).toHaveBeenCalledTimes(1)
    const [tenantId, patch] = h.updateTenant.mock.calls[0]
    expect(tenantId).toBe(ownerTenant.id)
    expect(patch.plan).toBeUndefined()
    expect(patch.billing_cycle).toBeUndefined()
    expect(patch.subscription_started_at).toBeUndefined()
    expect(patch.subscription_expires_at).toBeUndefined()
    expect(patch.metadata.promo).toEqual(
      expect.objectContaining({ price_usd: 127, expires_at: expect.any(String) }),
    )

    expect(h.preferenceCreate).toHaveBeenCalledWith(expect.objectContaining({
      body: expect.objectContaining({
        external_reference: expect.stringMatching(/^partner_sub_tenant-owner_\d+_annual_growth$/),
      }),
    }))
  })

  it('anual growth sin cupo: cobra precio de lista y no toca el tenant', async () => {
    h.promoEligible.mockResolvedValueOnce(false)

    const response = await POST(request({ plan: 'growth', billingCycle: 'annual' }))
    const data = await response.json()

    expect(response.status).toBe(200)
    expect(data.price_usd).toBe(255)
